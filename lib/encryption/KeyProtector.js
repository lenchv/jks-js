...
class KeyProtector {
	...
	recover(encryptedPrivateKeyInfo) {
		...
		const salt = sliceBuffer(protectedKey, 0, KeyProtector.SALT_LEN);
		...
		for (
			...
			if (i < numRounds - 1) {
				xorKey = Buffer.concat([
					sliceBuffer(xorKey, 0, xorOffset),
					digest
				]);
			} else {
				xorKey = Buffer.concat([
					sliceBuffer(xorKey, 0, xorOffset),
					sliceBuffer(digest, 0, encrKey.length - xorOffset)
				]);
			}
	   }
