	...
	describe('Java ' + javaVersion, function() {
		...
		describe('Keystore', () => {
			[
				...
				it('should extract certificates from ' + name, () => {
					const cert = toPem(fs.readFileSync(path.join(keyPath, 'keystore', name + '.jks')), 'password');
		...
		describe('Truststore', () => {
			it('cert should be extracted', () => {
				const cert = toPem(fs.readFileSync(path.join(keyPath, 'keystore', 'RSA_2048_truststore.jks')), 'password');
	
				expect(cert).has.key('jks-js');
				expect(cert['jks-js'].ca).to.be.eq(readCert('RSA_2048_keystore', keyPath));
			});
		});
	});
};
